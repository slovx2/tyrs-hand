...
echo "首次在 Chrome 默认 Profile 中打开 chrome://extensions，开启开发者模式，点击‘加载已解压的扩展程序’："
echo "$agent_root/unpacked-extension"
echo "确认扩展 ID 为 $extension_id，版本以该目录 manifest.json 为准。"
echo "扩展升级后点击‘重新加载’；Agent、Worker、Chrome 重启无需重复安装，无需重启 Chrome。"
echo "先在 Codex 中调用 Desktop 浏览器打开网页，再检查 http://127.0.0.1:8931/health 的 status=ready、connected=true 和扩展版本。"
echo "浏览器代理按需启动；首次工具调用前 connected=false 不代表扩展安装失败。"
