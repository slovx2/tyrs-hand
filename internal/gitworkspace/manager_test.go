...
func TestEnsureAndPublish(t *testing.T) {
	...
	run(t, workspace.WorktreePath, "git", "config", "user.name", "Agent")
	run(t, workspace.WorktreePath, "git", "config", "user.email", "agent@example.com")
	require.NoError(t, os.WriteFile(filepath.Join(workspace.WorktreePath, "change.txt"), []byte("change\n"), 0o600))
	run(t, workspace.WorktreePath, "git", "add", "change.txt")
	run(t, workspace.WorktreePath, "git", "commit", "-m", "change")
