...
func testGatewayHandlers(t *testing.T, ctx context.Context, db *sql.DB, manager *Manager, seed discordManagerSeed) {
	...
	require.NoError(t, db.QueryRowContext(ctx, `SELECT count(*) FROM official_turn_submissions
		WHERE conversation_id=$1`, conversationID).Scan(&queuedBeforeConfiguration))
	...
	_, err = db.ExecContext(ctx, `INSERT INTO official_thread_bindings(
		...
		VALUES($1,$2,$3,'thread-gateway-2001')`, seed.workspaceID, conversationID,
		seed.workspaceProjectID)
	require.NoError(t, err)
	...
	require.NoError(t, db.QueryRowContext(ctx, `SELECT status FROM official_turn_submissions
	...
	connector.onMessage(newMessageEvent(t, client, "2001", "5015", "continue after failed turn"))
	var acceptedMessageCount, queuedSubmissionCount int
	require.NoError(t, db.QueryRowContext(ctx, `SELECT count(*) FROM discord_input_messages
		WHERE message_id='5015'`).Scan(&acceptedMessageCount))
	require.Equal(t, 1, acceptedMessageCount)
	require.NoError(t, db.QueryRowContext(ctx, `SELECT count(*) FROM official_turn_submissions
		WHERE conversation_id=$1 AND discord_message_id='5015' AND status='queued'`,
		conversationID).Scan(&queuedSubmissionCount))
	require.Equal(t, 1, queuedSubmissionCount,
		"失败或停止过的官方 Thread 必须允许后续 Turn 继续")
}
