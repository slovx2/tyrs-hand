...
// ErrServerRequestUnclaimed 表示当前连接只观察该请求，不应向 App Server 写回响应。
// 其他官方客户端仍可在各自连接上取得交互权并完成请求。
var ErrServerRequestUnclaimed = errors.New("server request 未被当前连接认领")
...
// ConnectTransport 在已建立的透明 WebSocket 消息传输上完成官方初始化。
...
// DialSocketTransport 只建立官方 Unix WebSocket，不发送任何协议消息。
// 透明隧道由最终客户端自行完成 initialize/initialized。
...
func (c *SocketClient) Done() <-chan struct{} { return c.done }

func (c *SocketClient) Err() error { return c.failure() }
...
func (c *SocketClient) handleServerRequest(message rpcMessage) {
	...
	if errors.Is(err, ErrServerRequestUnclaimed) {
		return
	}
	if err != nil {
