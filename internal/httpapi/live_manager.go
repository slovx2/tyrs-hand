...
func (m *liveManager) waitEventType(ctx context.Context, sessionID uuid.UUID, typ string) error {
	ticker := time.NewTicker(20 * time.Millisecond)
	defer ticker.Stop()
	for {
		var exists bool
		err := m.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM live_events WHERE live_session_id=$1 AND event_type=$2)`, sessionID, typ).Scan(&exists)
		if err != nil {
			return err
		}
		if exists {
...
func (m *liveManager) injectHistory(ctx context.Context, sessionID uuid.UUID, history []live.InputMessage) error {
	for index, message := range history {
		if err := m.writeClientEvent(ctx, sessionID, liveHistoryAppendEvent(sessionID, index, message)); err != nil {
			return err
		}
	}
	return nil
...
func liveHistoryAppendEvent(sessionID uuid.UUID, index int, message live.InputMessage) map[string]any {
	text := ""
	...
	event := map[string]any{
		...
		"event_id": fmt.Sprintf("recover-%s-%d", sessionID, index),
		"channel":  "speakable",
...
func (m *liveManager) writeClientEvent(ctx context.Context, sessionID uuid.UUID, event map[string]any) error {
	...
	if err := m.persistEvent(ctx, sessionID, "client", event); err != nil {
		return err
	}
	return sideband.WriteJSON(ctx, event)
}
