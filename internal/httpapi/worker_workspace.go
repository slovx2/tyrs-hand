...
func (s *Server) workerWorkspaceState(c *gin.Context) {
	...
	err = tx.QueryRowContext(c.Request.Context(), `INSERT INTO repo_caches
		(repository_id, worker_id, path, status, last_fetch_at, last_used_at, error)
		VALUES ($1,$2,$3,$4,now(),now(),NULLIF($5,''))
		ON CONFLICT (worker_id, repository_id) WHERE worker_id IS NOT NULL
		...
		claimed.RepositoryID, worker.ID, request.CachePath, request.Status, request.Error).Scan(&cacheID)
	if err == nil {
		_, err = tx.ExecContext(c.Request.Context(), `INSERT INTO worktrees
			(work_item_id, repo_cache_id, worker_id, path, branch, base_sha, head_sha,
			...
			worker_id = EXCLUDED.worker_id, path = EXCLUDED.path,
			...
			error = EXCLUDED.error`, claimed.WorkItemID, cacheID, worker.ID, request.WorktreePath,
