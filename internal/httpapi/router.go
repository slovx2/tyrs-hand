...
func (s *Server) adminRouter() http.Handler {
	...
	authenticated.GET("/workers/:id/runtimes", s.getWorkerRuntimes)
	...
	authenticated.GET("/workers/:id/runtimes/:engine/config", s.getWorkerConfig)
	authenticated.PUT("/workers/:id/runtimes/:engine/config/agents", s.requireCSRF(), s.updateWorkerAgents)
	authenticated.PUT("/workers/:id/runtimes/:engine/config/provider", s.requireCSRF(), s.updateWorkerProvider)
	...
	authenticated.POST("/workers/:id/runtimes/:engine/restart", s.requireCSRF(), s.workerRuntimeRestart)
...
func (s *Server) securityHeaders() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Header("Content-Security-Policy", "default-src 'self'; connect-src 'self'; media-src 'self' blob:; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'")
		c.Next()
	}
}
