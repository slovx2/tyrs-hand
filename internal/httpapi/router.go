...
func (s *Server) adminRouter(includeWebhook bool) http.Handler {
	...
	authenticated.GET("/projects", s.listProjects)
	authenticated.POST("/projects", s.requireCSRF(), s.createProject)
	authenticated.POST("/projects/:id/retry", s.requireCSRF(), s.retryProject)
	authenticated.POST("/projects/:id/disable", s.requireCSRF(), s.disableProject)
	authenticated.POST("/projects/:id/enable", s.requireCSRF(), s.enableProject)
