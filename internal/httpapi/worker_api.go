...
func (s *Server) registerWorkerOperationRoutes(group *gin.RouterGroup) {
	...
	group.POST("/tunnels/claim", s.workerClaimAppServerTunnel)
	group.GET("/tunnels/:id/connect", s.workerConnectAppServerTunnel)
	group.POST("/materializations/claim", s.workerClaimMaterialization)
	group.GET("/materializations/:id/content", s.workerDownloadMaterialization)
	group.POST("/materializations/:id/complete", s.workerCompleteMaterialization)
	group.POST("/materializations/:id/fail", s.workerFailMaterialization)
...
func (s *Server) workerClaim(c *gin.Context) {
	...
	if request.Role != "github" {
		badRequest(c, errors.New("role 必须是 github；Workspace 交互通过官方 App Server 连接"))
		return
	}
	if !workerregistry.HasRole(worker, "github") {
	...
	for {
		...
		if err := s.db.QueryRowContext(c.Request.Context(), `SELECT count(*)
			FROM codex_turn_runs run JOIN codex_turn_intents intent
				ON intent.id=run.primary_intent_id
			WHERE run.worker_id=$1 AND run.active_slot=1
			  AND intent.source_type='github_work_item'`, worker.ID).
			Scan(&active); err != nil {
...
func (s *Server) claimedRemoteRun(ctx context.Context, workerID, runID uuid.UUID,
	...
	var source string
	var workItemID, repositoryID sql.NullString
	err := s.db.QueryRowContext(ctx, `SELECT r.control_id, r.primary_intent_id, r.id,
		...
		i.work_item_id::text,i.repository_id::text,i.agent_profile_id,i.sequence_no,
		i.attempt_count > 1 OR i.codex_submission_id IS NOT NULL,
		...
		WHERE r.id=$1 AND r.worker_id=$2 AND i.source_type='github_work_item'`,
		runID, workerID).Scan(
		&claimed.ControlID, &claimed.ID, &claimed.RunID, &claimed.LeaseEpoch, &source,
		&claimed.Attempt, &claimed.MaxAttempts, &workItemID, &repositoryID,
		&claimed.AgentProfileID, &claimed.Sequence,
		&claimed.Recovering, &claimed.SubmissionID, &claimed.ConfirmedTurnID,
		&claimed.ExternalThreadID)
	if err != nil {
		return nil, err
	}
	if source != codexcontrol.SourceGitHub {
		return nil, errors.New("worker Run 只接受 github_work_item")
	}
