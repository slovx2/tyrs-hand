		...
		if r.options.Controller == nil {
			if reservedArchiveLeader {
				r.finishArchive(reservedArchiveThreadID, reservedArchive, nil,
					errors.New("codex Hub 尚未配置 Desktop Control"))
			}
			return nil, &ProtocolError{Code: -32041,
				Message: "Codex Hub 尚未配置 Desktop Control"}
		}
		var err error
...
func (r *Hub) completeControlled(ctx context.Context, call Call, plan CallPlan,
...
func (r *Hub) subscribeCreatedThread(source *session, threadID string, ephemeral bool) {
	r.mu.Lock()
...
func (r *Hub) unsubscribe(ctx context.Context, source *session,
	params json.RawMessage,
) (json.RawMessage, error) {
...
func (r *Hub) anyDesktopSubscribed(threadID string) bool {
	r.mu.Lock()
	sessions := make([]*session, 0, len(r.sessions))
	for _, item := range r.sessions {
...
func (r *Hub) archiveThread(ctx context.Context, params json.RawMessage,
...
func (r *Hub) beginArchive(threadID string) (*archiveOperation, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Hub) runArchive(ctx context.Context, threadID string, params json.RawMessage,
...
func (r *Hub) archiveCanceled(operation *archiveOperation) bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	return operation.canceled
}

func (r *Hub) beginArchiveApply(operation *archiveOperation) bool {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Hub) cancelPendingArchive(threadID string) bool {
	r.mu.Lock()
...
func (r *Hub) threadActive(ctx context.Context, threadID string) (bool, error) {
	var result struct {
		Thread struct {
...
func (r *Hub) finishArchive(threadID string, operation *archiveOperation,
...
func (r *Hub) archivePending(threadID string) bool {
	if threadID == "" {
		return false
	}
	r.mu.Lock()
...
func (r *Hub) signalLifecycle(threadID string) {
	if threadID == "" {
		return
	}
	r.mu.Lock()
...
func (r *Hub) callIsEphemeral(method string, params json.RawMessage) bool {
...
func (r *Hub) markEphemeral(threadID string) {
	if threadID == "" {
		return
	}
	r.mu.Lock()
...
func (r *Hub) isEphemeral(threadID string) bool {
	if threadID == "" {
		return false
	}
	r.mu.Lock()
...
func (r *Hub) handleServerRequest(ctx context.Context,
...
func (r *Hub) firstDesktopAnswer(ctx context.Context, request codex.ServerRequest,
	threadID string,
) (any, error) {
...
func (r *Hub) workerForThread(threadID string) *session {
