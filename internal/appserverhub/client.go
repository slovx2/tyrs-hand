...
func (r *Hub) OpenClient(options ClientOptions) (*Client, error) {
	if options.Role == "" {
	...
	_, err := r.addSession(options.Role, nil, options.ServerRequestHandler, client, nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) Call(ctx context.Context, method string, params, result any) error {
	...
	response, err := c.hub.routeCall(ctx, c.session, method, raw)
	if err != nil {
...
func (c *Client) publish(event codex.Event) error {
	...
	if c.global.Load() {
		...
		case c.events <- event:
		default:
			return errors.New("hub worker 全局事件 backlog 已满")
		}
	}
