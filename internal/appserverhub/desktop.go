...
func (w *desktopWriter) write(message rpcMessage) error {
	...
	case <-w.done:
		return errSessionClosed
	default:
		return errors.New("hub Desktop 发送 backlog 已满")
	}
}
...
func (r *Hub) serveDesktop(response http.ResponseWriter, request *http.Request) {
	...
	go writer.run()
	s, err := r.addSession(RoleDesktop, writer.write, nil, nil, writer.close)
	if err != nil {
		writer.close()
		return
	}
...
func (r *Hub) handleDesktopCall(parent context.Context, source *session,
