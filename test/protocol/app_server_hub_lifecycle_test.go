...
func TestRealCodexHubArchiveWaitsForTurnAndRestoreReachesEveryClient(t *testing.T) {
	...
	root := temporaryDir(t, "tyrs-real-hub-lifecycle-")
...
`, responses.URL+"/v1")
	...
	waitForHubTurnCompleted(t, firstEvents.Events(), threadID, turnID)
	waitForHubTurnCompleted(t, secondEvents.Events(), threadID, turnID)
	select {
	case err := <-archiveDone:
		require.NoError(t, err)
	case <-time.After(10 * time.Second):
	...
	require.NoError(t, err)
	waitForHubTurnCompleted(t, workerEvents.Events(), threadID, resumedTurnID)
	waitForHubTurnCompleted(t, firstEvents.Events(), threadID, resumedTurnID)
