...
func TestRealCodexHubDiscordAndDesktopShareParticipantIdentity(t *testing.T) {
	...
	root := temporaryDir(t, "tyrs-real-hub-identity-")
...
`, responses.URL+"/v1")
	...
	hub, err := appserverhub.Start(context.Background(), appserverhub.Options{
		...
		Controller: identityHubController{participant: participant},
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	waitForHubTurnCompleted(t, workerEvents.Events(), threadID, turnID)
	...
	waitForHubTurnCompleted(t, desktopEvents.Events(), threadID, desktopTurn.Turn.ID)
	...
	waitForHubTurnCompleted(t, secondEvents.Events(), threadID, secondTurn.Turn.ID)
...
func TestRealCodexHubDesktopSelectsFastServiceTierWithAPIKeyUpstream(t *testing.T) {
	...
	root := temporaryDir(t, "tyrs-real-hub-service-tier-")
	home, workspace := filepath.Join(root, "home"), filepath.Join(root, "workspace")
	require.NoError(t, os.MkdirAll(home, 0o700))
	require.NoError(t, os.MkdirAll(workspace, 0o755))
...
`, responses.URL+"/v1")
	...
	hub, err := appserverhub.Start(context.Background(), appserverhub.Options{
		SocketPath: filepath.Join(root, "hub.sock"), UpstreamSocketPath: appSocket,
		Controller: serviceTierHubController{},
	})
	require.NoError(t, err)
	...
	waitForHubTurnCompleted(t, events.Events(), started.Thread.ID, turn.Turn.ID)
	select {
...
type serviceTierHubController struct{}

func (serviceTierHubController) PrepareCall(_ context.Context,
	call appserverhub.Call,
) (appserverhub.CallPlan, error) {
	return appserverhub.CallPlan{Params: call.Params, Forward: true}, nil
}

func (serviceTierHubController) CompleteCall(_ context.Context, call appserverhub.Call,
...
func (serviceTierHubController) ResolveInteractive(_ context.Context, _ codex.ServerRequest,
	answer json.RawMessage, _ appserverhub.Role,
) (bool, json.RawMessage, error) {
	return true, answer, nil
}

type identityHubController struct {
	participant participantidentity.Participant
}

func (c identityHubController) PrepareCall(_ context.Context,
	call appserverhub.Call,
) (appserverhub.CallPlan, error) {
	...
	return appserverhub.CallPlan{Params: params, Forward: true}, nil
}

func (identityHubController) CompleteCall(_ context.Context, _ appserverhub.Call,
	_ appserverhub.CallPlan, result json.RawMessage, cause error,
) (json.RawMessage, error) {
	return result, cause
}

func (identityHubController) ResolveInteractive(_ context.Context, _ codex.ServerRequest,
	answer json.RawMessage, _ appserverhub.Role,
) (bool, json.RawMessage, error) {
	return true, answer, nil
}

func TestRealCodexHubWorkerInitiatesAndOrdinaryDesktopRemainsFunctional(t *testing.T) {
	var responseNumber atomic.Int32
			...
			_, _ = fmt.Fprint(response, sse(
				...
				map[string]any{"type": "response.output_item.done", "item": map[string]any{
					...
					"action": map[string]any{"type": "exec",
						"command": []string{"/bin/sh", "-lc", "printf hub-shell"}},
	...
	root := temporaryDir(t, "tyrs-real-hub-multiclient-")
	home, workspace := filepath.Join(root, "home"), filepath.Join(root, "workspace")
	require.NoError(t, os.MkdirAll(home, 0o700))
	require.NoError(t, os.MkdirAll(workspace, 0o755))
	config := fmt.Sprintf(`model = "mock-model"
...
`, responses.URL+"/v1")
	...
	require.NoError(t, err)
	waitForRealHubEvent(t, desktopAll.Events(), "thread/started", threadID, "")
	workerEvents := worker.Subscribe(codex.ThreadFilter{ThreadID: threadID})
	t.Cleanup(workerEvents.Close)
	runtime := codex.NewRuntime(worker)
	...
	require.NoError(t, err)
	waitForHubTurnCompleted(t, workerEvents.Events(), threadID, firstTurnID)
	waitForHubTurnCompleted(t, desktopAll.Events(), threadID, firstTurnID)
	...
	waitForRealHubEvent(t, desktopAll.Events(), "thread/name/updated", threadID, "")
	waitForRealHubEvent(t, secondEvents.Events(), "thread/name/updated", threadID, "")
	require.NoError(t, desktop.Call(context.Background(), "thread/settings/update", map[string]any{
		"threadId": threadID, "approvalPolicy": "never",
		"permissions": ":danger-full-access",
	}, nil))
	waitForRealHubEvent(t, desktopAll.Events(), "thread/settings/updated", threadID, "")
	waitForRealHubEvent(t, secondEvents.Events(), "thread/settings/updated", threadID, "")
	...
	require.NoError(t, err)
	waitForHubTurnCompleted(t, workerEvents.Events(), threadID, turnID)
	waitForHubTurnCompleted(t, thirdEvents.Events(), threadID, turnID)
	select {
...
func TestRealCodexHubEphemeralThreadStaysDesktopOnly(t *testing.T) {
	responses := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
		response.Header().Set("Content-Type", "text/event-stream")
		response.Header().Set("Cache-Control", "no-cache")
		_, _ = fmt.Fprint(response, sse(
	...
	t.Cleanup(responses.Close)
	root := temporaryDir(t, "tyrs-real-hub-ephemeral-")
	home, workspace := filepath.Join(root, "home"), filepath.Join(root, "workspace")
	require.NoError(t, os.MkdirAll(home, 0o700))
	require.NoError(t, os.MkdirAll(workspace, 0o755))
	config := fmt.Sprintf(`model = "mock-model"
...
func (*realRecordingController) CompleteCall(_ context.Context, _ appserverhub.Call,
	_ appserverhub.CallPlan, result json.RawMessage, cause error,
) (json.RawMessage, error) {
	return result, cause
}
...
func waitForRealHubEvent(t *testing.T, events <-chan codex.Event, method, threadID,
	turnID string,
) {
	t.Helper()
	...
	for {
		...
		case <-timer.C:
			t.Fatalf("等待真实 Hub 事件 %s 超时", method)
		}
	}
}
