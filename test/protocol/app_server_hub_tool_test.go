...
func TestRealCodexHubDesktopAndWorkerDynamicToolRoundTrip(t *testing.T) {
	...
	responses := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = fmt.Fprint(response, sse(
			map[string]any{"type": "response.created", "response": map[string]any{"id": "hub-resp-2"}},
			map[string]any{"type": "response.output_item.done", "item": map[string]any{
				"type": "message", "role": "assistant", "id": "hub-message-1",
				"content": []map[string]any{{"type": "output_text", "text": "hub done"}},
			}}, completedResponse("hub-resp-2")))
	}))
	t.Cleanup(responses.Close)

	root := temporaryDir(t, "tyrs-real-hub-")
...
`, responses.URL+"/v1")), 0o600))
	...
	hubSocket := filepath.Join(root, "hub.sock")
	hub, err := appserverhub.Start(context.Background(), appserverhub.Options{
		SocketPath: hubSocket, UpstreamSocketPath: appSocket,
