...
docker compose -f compose.yaml -f compose.production.yaml up -d server worker discord
```
...
初始化完成后，在 Discord 页面确认 Gateway 为 `connected`，Outbox 和失败数为 `0`，再为需要使用的成员创建个人 Forum。
...
确认 PostgreSQL、Redis、Server、Worker 和 Discord 容器健康。在已安装仓库的 Issue 中评论 `@<app-slug> 检查当前问题并回复结果`，然后在管理后台确认 Webhook、Job、Thread 和 Worktree 状态。

启用 Discord 时，还应在个人 Forum 新建 Post，确认 Bot 返回工作区选择卡、任务状态持续更新并最终给出回复；随后回到管理后台确认 Gateway 仍为 `connected`，Outbox 没有待处理或失败积压。
