...
Codex Desktop 入站 SSH 与下一节供 Agent 访问外部机器的“受管 SSH”是两套独立能力。前者让 Desktop 进入某个 Discord 开发环境，后者把 Control 保存的远端凭据加载到 Worker 的 `ssh-agent`；两者的密钥、端口和信任边界不共享。

启用 Desktop 入站连接时，开发镜像必须在构建阶段提供 `sshd`、`ssh-keygen` 和 SFTP Server。系统不会在运行时尝试兼容安装缺失的软件。随后在管理后台为目标开发环境配置：

1. 一把 SSH 公钥。
2. 执行节点上的宿主端口。
3. 代表该凭据使用者的活跃 Discord 成员。

环境无论是否配置 SSH 都会永久运行。配置成功后，Codex Desktop 使用该主机和私钥通过 SSH 执行 `codex app-server proxy`；容器内薄 Relay 复用环境级 `CODEX_HOME` 和唯一 app-server，并把绑定成员作为 Desktop 发言身份。私钥只保存在 Desktop 所在机器，不上传到 Control 或 Worker。

Desktop 主 Thread 和 Turn 以可用性为先。首条 Desktop 输入、后续消息、进度和标题会异步投影到 Discord；Discord 或 Control 暂时失败不会阻塞 Desktop，恢复后由 Outbox 和对账流程补齐。Desktop 的模型、思考强度和 Service Tier 以 Desktop/app-server 当前设置为准，管理端默认参数只作用于 Discord 发起的会话。

系统创建的 Discord Post 使用七天不活跃自动隐藏。未锁定的 Discord 归档只表示隐藏，不改变 Codex Thread；`/codex archive` 才会等待当前 Turn 完成后真正归档 Codex 并锁定 Post，`/codex restore` 会恢复原 Thread 和 Post。
...
确认 PostgreSQL、Redis、Server 和 Discord 健康；执行节点为 `online` 且持续更新心跳；GitHub、Discord 默认节点正确；Agent 受管 SSH 为 `ready`；Chrome 扩展加载后浏览器状态为 `ready`；Worker 日志没有认证、协议、配置同步或租约循环错误。还要确认 Worker 上实际使用的 `compose.worker.yaml` 与当前发布制品一致，其协议版本与 `internal/workerprotocol.Version` 一致。

最后分别从真实 GitHub Issue/PR 和 Discord Forum 启动一次任务，在两条链路中各执行一次 SSH 操作和一次宿主 Chrome 前端验证。发布范围包含 Desktop/Relay 时，再通过配置的入站 SSH 验证 Desktop 建立会话和 Discord 投影。确认任务冻结并运行在预期节点，Chrome 能枚举和控制当前 Profile 的普通标签页且全程无确认弹窗，Gateway、Projection 与 Outbox 没有积压。未录入对应 SSH 凭证、未完成 Chrome 策略加载或未执行 Desktop 链路时，只能把对应项记为“未覆盖”，不能仅凭健康接口宣称完整 E2E 通过。
