...
状态卡、历史翻页、Plan、长回复和配置公告均读取原会话引擎；修改论坛默认值不会改变标识。
仍需完成实际三端接力和安装版客户端验收。

Claude 目标记录使用适配器 SQLite 持久化，归档、重启和原生 fork 后保持独立。
GOAL-001 验证原生 SDK 链路，GOAL-003 经真实 SSH 验证读写、分叉、删除和重启；
元数据操作不触发模型。自动续跑、目标上下文及预算/用量控制属于单独的 GOAL-002，
该用例尚未完成，不能以目标读写测试代替目标执行验收。
...
桌面创建会话后立即提交时，Worker 先完成会话、Run、提交确认，再补报事件与终态。
数据库拒绝变更已登记的提交/Turn ID，迟到的相同确认不能重新打开已完成任务。
恢复 Journal 超过并发上限时后台排队，心跳和活动命令领取继续运行；两个引擎仍共用
原有并发预算。手机审批、归档和待确认消息的异步更新检查所属连接，旧入口的迟到
结果不能修改当前入口的同 ID 记录。这些行为有确定性延迟和竞态回归用例。
