...
export default function LiveScreen() {
  ...
  const close = async () => { if (!link || !sessionId) return; try { await closeLiveSession(link, sessionId); setStatus("已关闭"); setSessionStatus("closed"); stopPeer(); } catch (reason) { setError(reason instanceof Error ? reason.message : "关闭失败"); setSessionStatus("failed"); stopPeer(); } };
