...
export type ConversationRow =
  | { kind: "turn"; key: string; turn: MobileTurn }
  | { kind: "request"; key: string; request: ServerRequest };

const turnRows = new WeakMap<MobileTurn, Extract<ConversationRow, { kind: "turn" }>>();
 ...
 */
export function conversationRows(turns: MobileTurn[], requests: ServerRequest[]): ConversationRow[] {
  return [...turns.map(turnRow), ...requests.map(requestRow)];
}

function turnRow(turn: MobileTurn): Extract<ConversationRow, { kind: "turn" }> {
