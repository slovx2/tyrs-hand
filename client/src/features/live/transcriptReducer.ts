...
function eventTurnRole(event: LiveTranscriptEvent): LiveTranscriptItem["role"] | undefined {
  let role: unknown
  if (event.turn && typeof event.turn === "object" && !Array.isArray(event.turn)) {
...
function eventTurnTranscript(event: LiveTranscriptEvent): string | undefined {
  if (!event.turn || typeof event.turn !== "object" || Array.isArray(event.turn)) return undefined
  const object = event.turn as Record<string, unknown>
  return nonEmptyString(object.transcript) ?? nonEmptyString(object.text)
}

function isTurnType(lowerType: string, phase: "created" | "delta" | "done"): boolean {
  return lowerType === `turn.${phase}` || lowerType.endsWith(`.turn.${phase}`)
}

function transcriptKey(event: LiveTranscriptEvent, role: TranscriptKind["role"], type: string, phase: TranscriptKind["phase"]): string {
...
function activeTurnKey(state: LiveTranscriptState, role: LiveTranscriptItem["role"]): string | undefined {
...
function finalizeTurn(
  ...
  const turnKey = findTurnPartialKey(state, role, turnId) ?? (role && turnId ? `${role}:turn:${turnId}` : undefined)
  const openKey = role ? `${role}:open` : undefined
  const text = transcript ?? (turnKey ? state.partial[turnKey]?.text : undefined) ?? (openKey ? state.partial[openKey]?.text : undefined) ?? ""
  const items = [...state.items]
  const partial = { ...state.partial }
  const finalized = { ...state.finalized }
  if (text && role) {
    items.push(withEventId(
      { role, text },
      id ?? (turnKey ? state.partial[turnKey]?.eventId : undefined) ?? (openKey ? state.partial[openKey]?.eventId : undefined),
    ))
  }
  if (turnKey) {
...
export function reduceLiveTranscript(state: LiveTranscriptState, event: LiveTranscriptEvent): LiveTranscriptState {
  ...
  if (isTurnType(lowerType, "created")) {
    ...
    delete partial[openKey]
    if (text) {
      partial[key] = withEventId({ role, text }, id ?? state.partial[openKey]?.eventId)
    }
  ...
  if (isTurnType(lowerType, "delta")) {
    ...
    const role = eventTurnRole(event)
    const turnId = eventTurnId(event)
    const key = findTurnPartialKey(state, role, turnId) ?? (role && turnId ? `${role}:turn:${turnId}` : undefined)
    ...
    return {
      ...
      partial: {
        ...state.partial,
        [key]: withEventId({ role: nextRole, text: (existing?.text ?? "") + delta }, id ?? existing?.eventId),
      },
    }
  }
  ...
  if (kind.phase === "delta" || kind.phase === "added") {
    if (kind.phase === "added" && activeTurnKey(state, kind.role) && key === `${kind.role}:open`) {
    ...
    return {
      ...
      partial: {
        ...state.partial,
        [key]: withEventId({ role: kind.role, text: (state.partial[key]?.text ?? "") + piece }, id),
      },
    }
  }
