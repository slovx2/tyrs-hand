  ...
  return (
    <main className="grid min-h-screen place-items-center p-8">
      <form className="panel w-full max-w-md" onSubmit={async (event) => {
        ...
        <label className="mt-5 block"><span className="label">邀请 Token</span><input className="field mt-1" value={token} onChange={(event) => setToken(event.target.value)} required /></label>
        <label className="mt-4 block"><span className="label">设置密码</span><input className="field mt-1" type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={8} required /></label>
