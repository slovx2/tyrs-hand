...
function containerId(value: unknown): string | undefined {
  return fieldFrom(objectRecord(value), [
    'item_id',
    'itemId',
    'response_id',
    'responseId',
...
function transcriptKind(type: string): TranscriptKind | null {
  ...
  if (
    ...
    return null
  const role =
    lower.includes('input_transcript') ||
    lower.includes('input_audio_transcription')
      ? 'user'
      : lower.includes('output_transcript') ||
          lower.includes('output_audio_transcript') ||
          lower.includes('output_text')
        ? 'assistant'
        : null
  if (!role) return null
...
function transcriptKey(
  ...
  const identity =
    ...
    event.response_id ??
    event.responseId ??
    containerId(event.item) ??
    containerId(event.response)
  ...
  if (kind.phase === 'delta' || kind.phase === 'added') {
    ...
    const piece =
      kind.phase === 'delta'
        ...
        : (knownText(event, ['delta']) ??
          knownText(event, ['text']) ??
          knownText(event, ['transcript']))
